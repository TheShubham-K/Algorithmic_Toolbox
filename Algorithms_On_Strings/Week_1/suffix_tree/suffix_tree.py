...
def build_suffix_tree(content):
    ...
    while j < n:
        match = branch.copy()
        add_new = True
        # check for branch(i = j); check for subbranch(i = rest string starting point)
        while len(match) > 0:
            while len(match) > 0:
                node = match.pop()
            ...
            if not add_new:
                ...
                if f >= e:
                    # not update i since new string not end, next while check for subbranch
...
def matching(i, content, target):
    f = target.front
    e = target.end
    # need to mark off target end from not match
